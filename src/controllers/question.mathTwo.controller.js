...
const QuestionmathTwoData = require("../models/question.model");
const { body, validationResult } = require("express-validator");

router.get("/", async (req, res) => {
  try {
    const data = await QuestionmathTwoData.find({}).lean().exec();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const question = await QuestionmathTwoData.create(req.body);
      return res
        .status(200)
...
router.patch("/:id", async (req, res) => {
  try {
    const question = await QuestionmathTwoData.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    )
...
router.delete("/:id", async (req, res) => {
  try {
    const question = await QuestionmathTwoData.findByIdAndDelete(req.params.id);
