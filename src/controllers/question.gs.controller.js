...
const QuestionGsData = require("../models/question.model");
const { body, validationResult } = require("express-validator");

router.get("/", async (req, res) => {
  try {
    const data = await QuestionGsData.find({}).lean().exec();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const question = await QuestionGsData.create(req.body);
      return res
        .status(200)
...
router.patch("/:id", async (req, res) => {
  try {
    const question = await QuestionGsData.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    )
...
router.delete("/:id", async (req, res) => {
  try {
    const question = await QuestionGsData.findByIdAndDelete(req.params.id);
