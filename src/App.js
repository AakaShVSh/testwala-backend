...
// /* ── Security headers ───────────────────────────────────────────────────────
//    Sets 15 HTTP headers (X-Frame-Options, HSTS, CSP, etc.) automatically.
// ──────────────────────────────────────────────────────────────────────────── */
// app.use(helmet());

// /* ── Rate limiters ──────────────────────────────────────────────────────────
//    Auth routes: 20 attempts per 15 min window (brute-force protection).
//    General API: 300 req per 15 min (basic DoS protection).
// ──────────────────────────────────────────────────────────────────────────── */
// const authLimiter = rateLimit({
//   windowMs: 15 * 60 * 1000,
//   max: 20,
...
// /* ── Input sanitization ─────────────────────────────────────────────────────
//    mongoSanitize: strips $ and . from req.body/query/params (NoSQL injection).
//    xss:           strips HTML tags from all string inputs.
//    hpp:           removes duplicate query params (HTTP param pollution).
// ──────────────────────────────────────────────────────────────────────────── */
// app.use(mongoSanitize());
// app.use(xss());
// app.use(hpp());

// /* ── Global ObjectId guard ──────────────────────────────────────────────────
//    Any route param named "id" that isn't a valid Mongo ObjectId returns a
//    clean 400 instead of a CastError 500 leaking stack traces.
// ──────────────────────────────────────────────────────────────────────────── */
// app.param("id", (req, res, next, id) => {
//   if (!mongoose.Types.ObjectId.isValid(id))
//     return res.status(400).json({ message: "Invalid ID format" });
...
// /* ── Routes ─────────────────────────────────────────────────────────────── */
// // Auth routes get tight rate limiting (brute-force protection)
// app.use("/auth/signin", authLimiter);
// app.use("/auth/signup", authLimiter);
// app.use("/auth/forgot-password", authLimiter);

// // All other routes get the general limiter
